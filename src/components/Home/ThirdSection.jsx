const ThirdSection = () => {
  return (
    <section
      class="bg-white dark:bg-gray-900 font-montserrat"
      data-aos="fade-up"
    >
      <div class="items-center max-w-screen-xl px-4 py-8 mx-auto lg:grid lg:grid-cols-4 lg:gap-16 xl:gap-24 lg:py-24 lg:px-6">
        <div class="col-span-2 mb-8">
          <p class="animate-fade-right animate-once text-lg font-medium text-principalAmber dark:text-principalAmber">
            Confianza en tus finanzas
          </p>
          <h2 class="animate-fade-right mt-3 mb-4 text-3xl font-extrabold tracking-tight text-gray-900 md:text-3xl dark:text-white">
            Portafolio de productos y servicios financieros
          </h2>
          <p class="animate-fade-right font-light text-gray-500 sm:text-xl dark:text-gray-400">
            El objetivo de ATMAN es convertirse en el líder del sector Fintech a
            través del portafolio de servicios y productos financieros digitales
            que permita maximizar el volumen de negocios y su rentabilidad.
          </p>
          {/* <div class="pt-6 mt-6 space-y-4 border-t border-gray-200 dark:border-gray-700">
                    <div>
                      <a href="#" class="animate-fade-right inline-flex items-center text-base font-medium text-principalAmber hover:text-purple-800 dark:text-principalAmber dark:hover:text-purple-700">
                        Posible link 1
                        <svg class="w-5 h-5 ml-1" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M12.293 5.293a1 1 0 011.414 0l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414-1.414L14.586 11H3a1 1 0 110-2h11.586l-2.293-2.293a1 1 0 010-1.414z" clip-rule="evenodd"></path></svg>
                      </a>
                    </div>
                    <div>
                      <a href="#" class="animate-fade-right inline-flex items-center text-base font-medium text-principalAmber hover:text-purple-800 dark:text-principalAmber dark:hover:text-purple-700">
                            Posible link 2
                          <svg class="w-5 h-5 ml-1" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M12.293 5.293a1 1 0 011.414 0l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414-1.414L14.586 11H3a1 1 0 110-2h11.586l-2.293-2.293a1 1 0 010-1.414z" clip-rule="evenodd"></path></svg>
                      </a>
                      </div>
                </div> */}
        </div>
        <div class="col-span-2 space-y-8 md:grid md:grid-cols-2 md:gap-12 md:space-y-0">
          <div>
            <svg
              class="animate-fade-down w-10 h-10 mb-2 md:w-12 md:h-12"
              fill="#d2a663"
              height="50px"
              width="50px"
              version="1.1"
              id="Layer_1"
              xmlns="http://www.w3.org/2000/svg"
              xmlns:xlink="http://www.w3.org/1999/xlink"
              viewBox="0 0 511.65 511.65"
              xml:space="preserve"
            >
              <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
              <g
                id="SVGRepo_tracerCarrier"
                stroke-linecap="round"
                stroke-linejoin="round"
              ></g>
              <g id="SVGRepo_iconCarrier">
                {" "}
                <g>
                  {" "}
                  <g>
                    {" "}
                    <g>
                      {" "}
                      <path d="M264.358,196.267v-8.463c9.376-1.908,17.834-6.907,24.049-14.249c11.053-13.071,8.549-32.902-5.025-43.47l-19.024-14.797 V68.776c9.943,3.514,17.067,12.989,17.067,24.138c0,4.713,3.821,8.533,8.533,8.533c4.713,0,8.533-3.82,8.533-8.533 c0-20.646-14.658-37.859-34.133-41.812v-8.436c0-4.713-3.82-8.533-8.533-8.533c-4.713,0-8.533,3.82-8.533,8.533v8.452 c-9.376,1.908-17.834,6.907-24.049,14.249c-11.054,13.072-8.549,32.914,5.029,43.473l19.02,14.794v46.511 c-9.941-3.515-17.067-12.993-17.067-24.137c0-4.713-3.821-8.533-8.533-8.533s-8.533,3.821-8.533,8.533 c0,20.64,14.659,37.858,34.133,41.811v8.447c0,4.713,3.82,8.533,8.533,8.533C260.538,204.8,264.358,200.979,264.358,196.267z M238.75,95.369c-6.051-4.706-7.154-13.447-2.478-18.978c2.965-3.502,6.767-6.112,11.02-7.614v33.236L238.75,95.369z M272.901,143.554c6.052,4.712,7.154,13.447,2.478,18.978c-2.965,3.502-6.767,6.112-11.02,7.614V136.91L272.901,143.554z"></path>{" "}
                      <path d="M366.758,375.117h-102.4v-25.989c47.87-4.306,85.333-44.478,85.333-93.477c0-4.713-3.821-8.533-8.533-8.533 c-31.764,0-59.819,15.745-76.8,39.861v-48.351c61.998-4.376,110.933-56.048,110.933-119.16C375.292,53.484,321.807,0,255.825,0 S136.358,53.484,136.358,119.467c0,63.112,48.935,114.784,110.933,119.16v48.351c-16.981-24.116-45.036-39.861-76.8-39.861 c-4.713,0-8.533,3.82-8.533,8.533c0,48.999,37.463,89.171,85.333,93.477v25.989h-102.4c-4.713,0-8.533,3.82-8.533,8.533v34.133 c0,4.713,3.82,8.533,8.533,8.533h10.718l23.776,79.252c1.083,3.609,4.405,6.081,8.173,6.081h136.533 c3.768,0,7.091-2.472,8.173-6.081l23.776-79.252h10.718c4.713,0,8.533-3.82,8.533-8.533V383.65 C375.292,378.937,371.471,375.117,366.758,375.117z M332.097,264.711c-3.496,29.817-24.04,54.378-51.635,63.703l15.53-15.53 c3.333-3.333,3.333-8.735,0-12.068c-3.332-3.332-8.735-3.332-12.068,0l-15.53,15.53 C277.72,288.751,302.28,268.208,332.097,264.711z M153.425,119.467c0-56.557,45.844-102.4,102.4-102.4s102.4,45.844,102.4,102.4 s-45.844,102.4-102.4,102.4S153.425,176.023,153.425,119.467z M179.553,264.711c29.849,3.5,54.429,24.084,63.732,51.724 l-15.679-15.629c-3.338-3.327-8.741-3.318-12.068,0.02s-3.318,8.741,0.02,12.068l15.543,15.492 C203.548,319.038,183.045,294.496,179.553,264.711z M317.743,494.583H193.907l-20.48-68.267h164.795L317.743,494.583z M358.225,409.25h-8.533H161.958h-8.533v-17.067h204.8V409.25z"></path>{" "}
                    </g>{" "}
                  </g>{" "}
                </g>{" "}
              </g>
            </svg>
            <h3 class="animate-fade-down mb-2 text-2xl font-bold dark:text-white">
              Créditos PYME
            </h3>
            {/* <p class="font-light text-gray-500 dark:text-gray-400">For Landwind, with zero maintenance downtime</p> */}
          </div>
          <div>
            <svg
              class="w-10 h-10 mb-2 md:w-12 md:h-12"
              width="50px"
              height="50px"
              viewBox="-0.5 0 25 25"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
              <g
                id="SVGRepo_tracerCarrier"
                stroke-linecap="round"
                stroke-linejoin="round"
              ></g>
              <g id="SVGRepo_iconCarrier">
                {" "}
                <path
                  d="M12.8702 16.97V18.0701C12.8702 18.2478 12.7995 18.4181 12.6739 18.5437C12.5482 18.6694 12.3778 18.74 12.2001 18.74C12.0224 18.74 11.852 18.6694 11.7264 18.5437C11.6007 18.4181 11.5302 18.2478 11.5302 18.0701V16.9399C11.0867 16.8668 10.6625 16.7051 10.2828 16.4646C9.90316 16.2241 9.57575 15.9097 9.32013 15.54C9.21763 15.428 9.16061 15.2817 9.16016 15.1299C9.16006 15.0433 9.17753 14.9576 9.21155 14.8779C9.24557 14.7983 9.29545 14.7263 9.35809 14.6665C9.42074 14.6067 9.49484 14.5601 9.57599 14.5298C9.65713 14.4994 9.7436 14.4859 9.83014 14.49C9.91602 14.4895 10.0009 14.5081 10.0787 14.5444C10.1566 14.5807 10.2254 14.6338 10.2802 14.7C10.6 15.1178 11.0342 15.4338 11.5302 15.6099V13.0701C10.2002 12.5401 9.53015 11.77 9.53015 10.76C9.55019 10.2193 9.7627 9.70353 10.1294 9.30566C10.4961 8.9078 10.9929 8.65407 11.5302 8.59009V7.47998C11.5302 7.30229 11.6007 7.13175 11.7264 7.0061C11.852 6.88045 12.0224 6.81006 12.2001 6.81006C12.3778 6.81006 12.5482 6.88045 12.6739 7.0061C12.7995 7.13175 12.8702 7.30229 12.8702 7.47998V8.58008C13.2439 8.63767 13.6021 8.76992 13.9234 8.96924C14.2447 9.16856 14.5226 9.43077 14.7402 9.73999C14.8284 9.85568 14.8805 9.99471 14.8901 10.1399C14.8928 10.2256 14.8783 10.3111 14.8473 10.3911C14.8163 10.4711 14.7696 10.5439 14.7099 10.6055C14.6502 10.667 14.5787 10.7161 14.4998 10.7495C14.4208 10.7829 14.3359 10.8001 14.2501 10.8C14.1607 10.7989 14.0725 10.7787 13.9915 10.7407C13.9104 10.7028 13.8384 10.648 13.7802 10.5801C13.5417 10.2822 13.2274 10.054 12.8702 9.91992V12.1699L13.1202 12.27C14.3902 12.76 15.1802 13.4799 15.1802 14.6299C15.163 15.2399 14.9149 15.8208 14.4862 16.2551C14.0575 16.6894 13.4799 16.9449 12.8702 16.97ZM11.5302 11.5901V9.96997C11.3688 10.0285 11.2298 10.1363 11.1329 10.2781C11.0361 10.4198 10.9862 10.5884 10.9902 10.76C10.9984 10.93 11.053 11.0945 11.1483 11.2356C11.2435 11.3767 11.3756 11.4889 11.5302 11.5601V11.5901ZM13.7302 14.6599C13.7302 14.1699 13.3902 13.8799 12.8702 13.6599V15.6599C13.1157 15.6254 13.3396 15.5009 13.4985 15.3105C13.6574 15.1202 13.74 14.8776 13.7302 14.6299V14.6599Z"
                  fill="#d2a663"
                ></path>{" "}
                <path
                  d="M12.58 3.96997H6C4.93913 3.96997 3.92178 4.39146 3.17163 5.1416C2.42149 5.89175 2 6.9091 2 7.96997V17.97C2 19.0308 2.42149 20.0482 3.17163 20.7983C3.92178 21.5485 4.93913 21.97 6 21.97H18C19.0609 21.97 20.0783 21.5485 20.8284 20.7983C21.5786 20.0482 22 19.0308 22 17.97V11.8999"
                  stroke="#d2a663"
                  stroke-width="1.5"
                  stroke-linecap="round"
                  stroke-linejoin="round"
                ></path>{" "}
                <path
                  d="M16.3398 8.57992L21.9998 2.91992"
                  stroke="#d2a663"
                  stroke-width="1.5"
                  stroke-linecap="round"
                  stroke-linejoin="round"
                ></path>{" "}
                <path
                  d="M17.4805 2.91992H22.0005V7.44992"
                  stroke="#d2a663"
                  stroke-width="1.5"
                  stroke-linecap="round"
                  stroke-linejoin="round"
                ></path>{" "}
              </g>
            </svg>
            <h3 class="mb-2 text-2xl font-bold dark:text-white">
              Domiciliación
            </h3>
            {/* <p class="font-light text-gray-500 dark:text-gray-400">Trusted by over 600 milion users around the world</p> */}
          </div>
          <div>
            <svg
              class="w-10 h-10 mb-2 md:w-12 md:h-12"
              fill="#d2a663"
              version="1.1"
              id="Layer_1"
              xmlns="http://www.w3.org/2000/svg"
              xmlns:xlink="http://www.w3.org/1999/xlink"
              viewBox="0 0 512 512"
              xml:space="preserve"
              width="50px"
              height="50px"
            >
              <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
              <g
                id="SVGRepo_tracerCarrier"
                stroke-linecap="round"
                stroke-linejoin="round"
              ></g>
              <g id="SVGRepo_iconCarrier">
                {" "}
                <g>
                  {" "}
                  <g>
                    {" "}
                    <path d="M381.388,65.43V28.735C381.388,12.89,368.498,0,352.653,0H133.224C117.38,0,104.49,12.89,104.49,28.735V512h276.898 v-21.545c14.624-1.325,26.122-13.647,26.122-28.61V94.041C407.51,79.078,396.012,66.755,381.388,65.43z M154.122,15.673h177.633 V175.02c0,7.203-5.859,13.061-13.061,13.061h-151.51c-7.202,0-13.061-5.859-13.061-13.061V15.673z M171.299,496.327l3.634-8.359 h137.054l3.634,8.359H171.299z M365.714,496.327h-33.001l-10.449-24.033H164.658l-10.449,24.033h-34.046V28.735 c0-7.202,5.859-13.061,13.061-13.061h5.224V175.02c0,15.845,12.89,28.735,28.735,28.735h151.51 c15.845,0,28.735-12.89,28.735-28.735V15.673h5.224c7.202,0,13.061,5.859,13.061,13.061V496.327z M391.837,461.845 c0,6.308-4.494,11.585-10.449,12.798v-393.4c5.955,1.213,10.449,6.49,10.449,12.798V461.845z"></path>{" "}
                  </g>{" "}
                </g>{" "}
                <g>
                  {" "}
                  <g>
                    {" "}
                    <path d="M331.755,399.174h-20.898c-12.964,0-23.51,10.546-23.51,23.51s10.546,23.51,23.51,23.51h20.898 c12.964,0,23.51-10.546,23.51-23.51S344.719,399.174,331.755,399.174z M331.755,430.521h-20.898c-4.322,0-7.837-3.515-7.837-7.837 c0-4.322,3.515-7.837,7.837-7.837h20.898c4.322,0,7.837,3.515,7.837,7.837C339.592,427.005,336.077,430.521,331.755,430.521z"></path>{" "}
                  </g>{" "}
                </g>{" "}
                <g>
                  {" "}
                  <g>
                    {" "}
                    <path d="M175.02,226.765h-20.898c-12.964,0-23.51,10.546-23.51,23.51c0,12.964,10.546,23.51,23.51,23.51h20.898 c12.964,0,23.51-10.546,23.51-23.51C198.531,237.312,187.984,226.765,175.02,226.765z M175.02,258.112h-20.898 c-4.322,0-7.837-3.515-7.837-7.837c0-4.322,3.515-7.837,7.837-7.837h20.898c4.322,0,7.837,3.515,7.837,7.837 C182.857,254.597,179.342,258.112,175.02,258.112z"></path>{" "}
                  </g>{" "}
                </g>{" "}
                <g>
                  {" "}
                  <g>
                    {" "}
                    <path d="M253.388,226.765H232.49c-12.964,0-23.51,10.546-23.51,23.51c0,12.964,10.546,23.51,23.51,23.51h20.898 c12.964,0,23.51-10.546,23.51-23.51C276.898,237.312,266.352,226.765,253.388,226.765z M253.388,258.112H232.49 c-4.322,0-7.837-3.515-7.837-7.837c0-4.322,3.515-7.837,7.837-7.837h20.898c4.322,0,7.837,3.515,7.837,7.837 C261.224,254.597,257.709,258.112,253.388,258.112z"></path>{" "}
                  </g>{" "}
                </g>{" "}
                <g>
                  {" "}
                  <g>
                    {" "}
                    <path d="M331.755,226.765h-20.898c-12.964,0-23.51,10.546-23.51,23.51c0,12.964,10.546,23.51,23.51,23.51h20.898 c12.964,0,23.51-10.546,23.51-23.51C355.265,237.312,344.719,226.765,331.755,226.765z M331.755,258.112h-20.898 c-4.322,0-7.837-3.515-7.837-7.837c0-4.322,3.515-7.837,7.837-7.837h20.898c4.322,0,7.837,3.515,7.837,7.837 C339.592,254.597,336.077,258.112,331.755,258.112z"></path>{" "}
                  </g>{" "}
                </g>{" "}
                <g>
                  {" "}
                  <g>
                    {" "}
                    <path d="M175.02,284.235h-20.898c-12.964,0-23.51,10.546-23.51,23.51c0,12.964,10.546,23.51,23.51,23.51h20.898 c12.964,0,23.51-10.546,23.51-23.51C198.531,294.781,187.984,284.235,175.02,284.235z M175.02,315.582h-20.898 c-4.322,0-7.837-3.515-7.837-7.837c0-4.322,3.515-7.837,7.837-7.837h20.898c4.322,0,7.837,3.515,7.837,7.837 C182.857,312.066,179.342,315.582,175.02,315.582z"></path>{" "}
                  </g>{" "}
                </g>{" "}
                <g>
                  {" "}
                  <g>
                    {" "}
                    <path d="M253.388,284.235H232.49c-12.964,0-23.51,10.546-23.51,23.51c0,12.964,10.546,23.51,23.51,23.51h20.898 c12.964,0,23.51-10.546,23.51-23.51C276.898,294.781,266.352,284.235,253.388,284.235z M253.388,315.582H232.49 c-4.322,0-7.837-3.515-7.837-7.837c0-4.322,3.515-7.837,7.837-7.837h20.898c4.322,0,7.837,3.515,7.837,7.837 C261.224,312.066,257.709,315.582,253.388,315.582z"></path>{" "}
                  </g>{" "}
                </g>{" "}
                <g>
                  {" "}
                  <g>
                    {" "}
                    <path d="M331.755,284.235h-20.898c-12.964,0-23.51,10.546-23.51,23.51c0,12.964,10.546,23.51,23.51,23.51h20.898 c12.964,0,23.51-10.546,23.51-23.51C355.265,294.781,344.719,284.235,331.755,284.235z M331.755,315.582h-20.898 c-4.322,0-7.837-3.515-7.837-7.837c0-4.322,3.515-7.837,7.837-7.837h20.898c4.322,0,7.837,3.515,7.837,7.837 C339.592,312.066,336.077,315.582,331.755,315.582z"></path>{" "}
                  </g>{" "}
                </g>{" "}
                <g>
                  {" "}
                  <g>
                    {" "}
                    <path d="M175.02,341.705h-20.898c-12.964,0-23.51,10.546-23.51,23.51s10.546,23.51,23.51,23.51h20.898 c12.964,0,23.51-10.546,23.51-23.51S187.984,341.705,175.02,341.705z M175.02,373.052h-20.898c-4.322,0-7.837-3.515-7.837-7.837 c0-4.322,3.515-7.837,7.837-7.837h20.898c4.322,0,7.837,3.515,7.837,7.837C182.857,369.535,179.342,373.052,175.02,373.052z"></path>{" "}
                  </g>{" "}
                </g>{" "}
                <g>
                  {" "}
                  <g>
                    {" "}
                    <path d="M253.388,341.705H232.49c-12.964,0-23.51,10.546-23.51,23.51s10.546,23.51,23.51,23.51h20.898 c12.964,0,23.51-10.546,23.51-23.51S266.352,341.705,253.388,341.705z M253.388,373.052H232.49c-4.322,0-7.837-3.515-7.837-7.837 c0-4.322,3.515-7.837,7.837-7.837h20.898c4.322,0,7.837,3.515,7.837,7.837C261.224,369.535,257.709,373.052,253.388,373.052z"></path>{" "}
                  </g>{" "}
                </g>{" "}
                <g>
                  {" "}
                  <g>
                    {" "}
                    <path d="M331.755,341.705h-20.898c-12.964,0-23.51,10.546-23.51,23.51s10.546,23.51,23.51,23.51h20.898 c12.964,0,23.51-10.546,23.51-23.51S344.719,341.705,331.755,341.705z M331.755,373.052h-20.898c-4.322,0-7.837-3.515-7.837-7.837 c0-4.322,3.515-7.837,7.837-7.837h20.898c4.322,0,7.837,3.515,7.837,7.837C339.592,369.535,336.077,373.052,331.755,373.052z"></path>{" "}
                  </g>{" "}
                </g>{" "}
                <g>
                  {" "}
                  <g>
                    {" "}
                    <path d="M175.02,399.174h-20.898c-12.964,0-23.51,10.546-23.51,23.51s10.546,23.51,23.51,23.51h20.898 c12.964,0,23.51-10.546,23.51-23.51S187.984,399.174,175.02,399.174z M175.02,430.521h-20.898c-4.322,0-7.837-3.515-7.837-7.837 c0-4.322,3.515-7.837,7.837-7.837h20.898c4.322,0,7.837,3.515,7.837,7.837C182.857,427.005,179.342,430.521,175.02,430.521z"></path>{" "}
                  </g>{" "}
                </g>{" "}
                <g>
                  {" "}
                  <g>
                    {" "}
                    <path d="M253.388,399.174H232.49c-12.964,0-23.51,10.546-23.51,23.51s10.546,23.51,23.51,23.51h20.898 c12.964,0,23.51-10.546,23.51-23.51S266.352,399.174,253.388,399.174z M253.388,430.521H232.49c-4.322,0-7.837-3.515-7.837-7.837 c0-4.322,3.515-7.837,7.837-7.837h20.898c4.322,0,7.837,3.515,7.837,7.837C261.224,427.005,257.709,430.521,253.388,430.521z"></path>{" "}
                  </g>{" "}
                </g>{" "}
                <g>
                  {" "}
                  <g>
                    {" "}
                    <path d="M164.571,67.918v109.714h156.735V67.918H164.571z M305.633,161.959H180.245V83.592h125.388V161.959z"></path>{" "}
                  </g>{" "}
                </g>{" "}
                <g>
                  {" "}
                  <g>
                    {" "}
                    <rect
                      x="172.408"
                      y="36.049"
                      width="141.061"
                      height="15.673"
                    ></rect>{" "}
                  </g>{" "}
                </g>{" "}
              </g>
            </svg>
            <h3 class="mb-2 text-2xl font-bold dark:text-white">
              Pago electrónico
            </h3>
            {/* <p class="font-light text-gray-500 dark:text-gray-400">Have used Landwind to create functional websites</p> */}
          </div>
          <div>
            <svg
              class="w-10 h-10 mb-2 md:w-12 md:h-12"
              fill="#d2a663"
              width="50px"
              height="50px"
              viewBox="0 0 64 64"
              xmlns="http://www.w3.org/2000/svg"
              stroke="#d2a663"
            >
              <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
              <g
                id="SVGRepo_tracerCarrier"
                stroke-linecap="round"
                stroke-linejoin="round"
              ></g>
              <g id="SVGRepo_iconCarrier">
                {" "}
                <g data-name="38 credit card" id="_38_credit_card">
                  {" "}
                  <path d="M55.5,15.21H8.5a5.008,5.008,0,0,0-5,5V43.79a5.008,5.008,0,0,0,5,5h47a5.008,5.008,0,0,0,5-5V20.21A5.008,5.008,0,0,0,55.5,15.21Zm3,28.58a3.009,3.009,0,0,1-3,3H8.5a3.009,3.009,0,0,1-3-3V27.4h53Zm0-18.39H5.5V23.32h53Zm0-4.08H5.5V20.21a3.009,3.009,0,0,1,3-3h47a3.009,3.009,0,0,1,3,3Z"></path>{" "}
                  <path d="M48.3,30.49a6.074,6.074,0,0,0-3.58,1.16,5.986,5.986,0,0,0-3.55-1.16,6.1,6.1,0,1,0,3.55,11.03A6.095,6.095,0,1,0,48.3,30.49ZM41.17,40.68a4.1,4.1,0,1,1,4.09-4.1A4.106,4.106,0,0,1,41.17,40.68Zm7.13,0a4.069,4.069,0,0,1-2.14-.61,6.071,6.071,0,0,0,0-6.97,4.092,4.092,0,1,1,2.14,7.58Z"></path>{" "}
                  <path d="M10.611,36.564h8.148a1,1,0,0,0,0-2H10.611a1,1,0,1,0,0,2Z"></path>{" "}
                  <path d="M20.8,38.639H10.611a1,1,0,1,0,0,2H20.8a1,1,0,0,0,0-2Z"></path>{" "}
                </g>{" "}
              </g>
            </svg>
            <h3 class="mb-2 text-2xl font-bold dark:text-white">
              Tarjeta de crédito
            </h3>
            {/* <p class="font-light text-gray-500 dark:text-gray-400">Transactions per day</p> */}
          </div>
          <div>
            <svg
              class="w-10 h-10 mb-2 md:w-12 md:h-12"
              width="50px"
              height="50px"
              viewBox="0 0 64 64"
              xmlns="http://www.w3.org/2000/svg"
              stroke-width="3"
              stroke="#d2a663"
              fill="none"
            >
              <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
              <g
                id="SVGRepo_tracerCarrier"
                stroke-linecap="round"
                stroke-linejoin="round"
              ></g>
              <g id="SVGRepo_iconCarrier">
                <path
                  d="M55.47,31.14A23.51,23.51,0,0,1,12.69,45.6"
                  stroke-linecap="round"
                ></path>
                <path
                  d="M8.46,32.74a24,24,0,0,1,.42-5,23.51,23.51,0,0,1,42.29-9.14"
                  stroke-linecap="round"
                ></path>
                <polyline
                  points="40.6 17.6 51.45 18.87 52.53 8.69"
                  stroke-linecap="round"
                ></polyline>
                <polyline
                  points="23.05 46.33 12.21 45.06 11.12 55.24"
                  stroke-linecap="round"
                ></polyline>
                <path d="M39,25.57a7.09,7.09,0,0,0-6.65-4.29c-6,0-6.21,4.29-6.21,4.29s-.9,5.28,6.43,5.85C40.18,32,39,37.26,39,37.26s-.78,4.58-6.43,4.87-7.41-5.65-7.41-5.65"></path>
                <line x1="32.33" y1="17.48" x2="32.33" y2="46.52"></line>
              </g>
            </svg>
            <h3 class="mb-2 text-2xl font-bold dark:text-white">
              Descuento por nómina
            </h3>
            {/* <p class="font-light text-gray-500 dark:text-gray-400">Transactions per day</p> */}
          </div>
          <div>
            <svg
              class="w-10 h-10 mb-2 md:w-12 md:h-12"
              fill="#d2a663"
              height="50px"
              width="50px"
              version="1.1"
              id="Layer_1"
              xmlns="http://www.w3.org/2000/svg"
              xmlns:xlink="http://www.w3.org/1999/xlink"
              viewBox="0 0 600 602.2"
              enable-background="new 0 0 600 602.2"
              xml:space="preserve"
            >
              <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
              <g
                id="SVGRepo_tracerCarrier"
                stroke-linecap="round"
                stroke-linejoin="round"
              ></g>
              <g id="SVGRepo_iconCarrier">
                {" "}
                <path d="M122.2,234c-61.8,0-112-50.2-112-112s50.5-112,112-112c61.8,0,112,50.2,112,112S184.1,234,122.2,234z M122.2,31.2 c-49.9,0-90.7,40.8-90.7,90.7s40.8,90.7,90.7,90.7s90.7-40.8,90.7-90.7S172.5,31.2,122.2,31.2z M395.4,426 c20.2-25.8,16.7-63.4-10.2-84.5c-25.8-20.2-64.3-15.6-84.5,10.2s-16.7,63.4,10.2,84.5C336.7,456.2,375.2,451.6,395.4,426z M322.5,398.4c-8.1-6.5-10.4-16.5-5.1-27.4l-8.1-6.5l7.2-9.3l7.2,5.6c3.3-4.2,9.3-7.7,13.7-10.9l7.9,11.1c-2.6,1.2-7,4.6-11.8,10.7 c-3.9,5.1-4.4,9.8-1.4,12.3c3,2.3,7.7,0.9,16.3-3.7c12.1-7,20.4-9.1,28.6-2.6c6.5,6.7,7.7,17.6,3.3,27.6l8.1,6.5l-7.2,9.3l-7.2-5.6 c-3.3,4.2-11.8,10.7-15.1,13l-7.9-11.1c4.4-1.4,11.4-6,15.3-11.1c4.9-6.3,5.1-10.9,1.2-14.2c-3-2.3-6.7-2.1-15.6,2.8 C341.3,401.6,330.7,404.9,322.5,398.4z M110.4,143.8c-3.2-1.9-5.1-5.4-5.1-9.2V60.9c0-5.9,4.9-10.5,10.5-10.5s10.5,4.9,10.5,10.5 v55.9l51.8-28.3c5.1-2.7,11.6-0.8,14.3,4.3c2.7,5.1,0.8,11.6-4.3,14.3l-67.5,36.7C119.3,144.9,113.9,145.7,110.4,143.8z M451.4,186.6l-35.1,39.5l-37.8-29.5l-93.8,120.3c-1.9-1.2-7.4-3.7-7.9-4.4c-20-9.3-46.9-8.4-66.4,1.6L7.4,427.4v150.5L97.3,534 c40.4,19,86.6,14.2,123.8-8.8l87.8,68.5l26.2-33.7l20.7,18.3l236.8-265.9L451.4,186.6z M329.3,540.5c-17-8.8-38.1-6-52,7.2 l-69.9-54.8c-15.3,12.3-33.9,18.8-52.9,18.8c-8.6,0-17-1.4-25.3-3.9c-4.2-1.4-6.5-5.8-5.3-10c1.4-4.2,5.8-6.5,10-5.3 c20.7,6.5,43.2,2.8,60.6-9.8l0,0c4.4-3.3,9.1-7.2,13.9-12.5l21.4-25.3l61.8,48.5c10.7,7.4,23.2,5.6,30.2-2.8 c7.4-10.7,5.6-23.5-2.8-30.2l-111-89.4c-3.3-2.6-3.7-7.4-1.2-10.7c2.6-3.3,7.4-3.7,10.7-1.2l34.1,27.4L359,248.4 c16.5,8.6,36.7,6,50.4-6.3l64.8,50.4c-8.8,17-5.8,37.8,7.4,51.8L329.3,540.5z M365.5,520.8l162.1-208.1L434,239.8 c14.9,6.5,32.5,4.4,45.5-5.8l61.3,54.6c-9.8,16.3-8.4,37.4,3.9,52L379.6,526.1C375.2,523.8,370.6,521.9,365.5,520.8z"></path>{" "}
              </g>
            </svg>
            <h3 class="mb-2 text-2xl font-bold dark:text-white">
              Adelanto de nómina
            </h3>
            {/* <p class="font-light text-gray-500 dark:text-gray-400">Transactions per day</p> */}
          </div>
        </div>
      </div>
    </section>
  );
};

export default ThirdSection;
